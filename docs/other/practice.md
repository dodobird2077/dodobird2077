...
2. `列表`（List）：有序的字符串列表，可以在头部或尾部进行添加、删除、获取元素。

3. `集合`（Set）：无序的字符串集合，不允许重复元素，可以进行添加、删除、判断是否存在等操作。

4. `有序集合`（Sorted Set）：与集合类似，但每个元素都关联一个分数，可以根据分数进行排序，支持添加、删除、获取元素，并可以根据分数范围获取元素。

5. `哈希`（Hash）：类似于`关联数组`，存储键值对的无序散列表，可以添加、获取、删除单个字段，也可以获取所有字段。

#### 下面是前5个Redis数据类型的常见操作方法：
...
2. 列表（List）：
   - 在列表头部添加元素：`LPUSH key element`
   - 在列表尾部添加元素：`RPUSH key element`
   - 获取列表长度：`LLEN key`
   - 获取指定范围的元素：`LRANGE key start stop`
   - 移除并返回列表的第一个元素：`LPOP key`

3. 集合（Set）：
   - 添加一个或多个元素到集合：`SADD key element1 element2 ...`
   - 获取集合的成员数：`SCARD key`
   - 判断元素是否在集合中：`SISMEMBER key element`
   - 移除一个或多个元素：`SREM key element1 element2 ...`
   - 获取集合中的所有成员：`SMEMBERS key`

4. 有序集合（Sorted Set）：
   - 添加一个元素到有序集合：`ZADD key score element`
   - 获取有序集合的成员数：`ZCARD key`
   - 根据分数范围获取元素：`ZRANGEBYSCORE key min max`
   - 获取有序集合中指定范围的元素：`ZRANGE key start stop`
   - 移除一个或多个元素：`ZREM key element1 element2 ...`

5. 哈希（Hash）：
   - 设置哈希字段的值：`HSET key field value`
   - 获取哈希字段的值：`HGET key field`
   - 获取哈希中所有字段的值：`HGETALL key`
   - 删除一个或多个字段：`HDEL key field1 field2 ...`
   - 获取哈希中字段的数量：`HLEN key`


#### 除了以上常见的数据类型，Redis还提供了一些其他的数据结构和功能，如：

6. 位图（Bitmap）：可以对位进行操作的数据结构，支持位的设置、获取、计数等操作。

7. 地理空间索引（Geospatial Index）：支持地理位置数据的存储和查询，可以进行附近位置搜索等操作。

8. HyperLogLog：用于估计一个集合的基数（不重复元素的数量），支持添加元素和计数操作。

9. 发布/订阅（Pub/Sub）：支持消息发布和订阅功能，可以实现消息队列和实时通信等应用。
